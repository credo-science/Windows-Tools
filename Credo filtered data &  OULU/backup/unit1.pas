...
uses
  ...
  Spin, TADrawUtils, TACustomSeries, TADbSource,
...
var
  val: string;
begin
  // filename := 'F:\Python37-32\a\credo-data-export\credo-analysis-image\wyniki\wyn.db';
  val := spinedit1.Value.ToString;

  filename :=
    'po_filtrze_tylko_tmestamp.sqlite';
  ...
  begin
    ...
    //   'select   strftime("%Y-%m-%d %H:",datetime(a.timestamp/1000,"unixepoch")) ||  case when ((cast(strftime("%M", datetime(a.timestamp/1000,"unixepoch")) as int) / 15) * 15) = 0 then "0" else "" end ||  ((cast(strftime("%M", datetime(a.timestamp/1000,"unixepoch")) as int) / 15) * 15) || ":00" period,  count(*) counter from detections  as a WHERE  ((device_id=1312) or (device_id=1343) or (device_id=1414) or team_id=1) and ((datetime(a.timestamp/1000,"unixepoch")> "'+data1+' 00:00:00" AND datetime(a.timestamp/1000,"unixepoch")< "'+data2+' 00:00:00" )) group by period ORDER by datetime(timestamp/1000,"unixepoch")';
    //memo3.Lines.Text := SQLQuery1.SQL.Text;
    SQLQuery1.Close;
    SQLQuery1.Open;
...
begin
 ...
 { if OpenDialog1.Execute then
  ...
  begin
    ...
    SQLQuery2.SQL.Text :=
      ('select   * from oulu1 where timeou BETWEEN "' + data1 + ' 00:00:00" and "' +
      data2 + ' 00:00:00"');
    SQLQuery2.Close;
    SQLQuery2.Open;
...
begin
  filename := 'F:\Python37-32\a\credo-data-export\credo-analysis-image\wyniki\eq3.sqlite';
 ...
 { if OpenDialog1.Execute then
  ...
  begin
    ...
    SQLQuery3.SQL.Text :=
      ('select   * from quereq where (mag>5) and time BETWEEN "' +
...
var
  ds: TDataset;
  fperiod, fcounter, fpressure: TField;
  x, y: double;
begin
